...
`select` certainly a nicer syntax than something like:
...
It may seem to be a surprise that these last couple of examples worked. But it really isn't because we are using the same technique that R uses when is does a standard evaluation of an expression. It walks through the expression using the head and tail of each quosures it finds.
...
There are other ways to make `quosures` and use them... we've only look at a couple of the functions that R has for working with quosures. `make_part_number` should do error checking. `Quosures` can handle much more complicated expressions than the part number we've started with here. `Quosures` can also be used to analyze an expression. But all that is for a later tutorials too.
